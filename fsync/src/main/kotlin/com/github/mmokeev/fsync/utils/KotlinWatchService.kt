 ...
 * @param [tag] - any state of data that should be associated with this channel
 ...
 * @param [tag] - any state of data that should be associated with this channel, optional
 */
class KWatchChannel(
    val file: File,
        ...
        scope.launch(Dispatchers.IO) {
            ...
            channel.send(
                KWatchEvent(
                    file = path.toFile(),
                    tag = tag,
                    state = KWatchEvent.State.Initialized
                )
            )
            ...
            while (!isClosedForSend) {
                ...
                monitorKey.pollEvents().forEach {
                    ...
                    if (mode == Mode.SingleFile && file.absolutePath != file.absolutePath) {
                        return@forEach
                    }

                    val eventType = when (it.kind()) {
                        ENTRY_CREATE -> KWatchEvent.State.Created
                        ENTRY_DELETE -> KWatchEvent.State.Deleted
                        else -> KWatchEvent.State.Modified
                    }
            ...
            logger.warn { "[KWatchChannel] Finished sending messages" }
        }
    }
...
data class KWatchEvent(
     ...
     */
    enum class State(val kind: String) {
        /**
